...
kotlin {
    ...
    val xcf = XCFramework("Ctor")

    // Apple desktop
    macosArm64 {
        binaries.framework { baseName = "Ctor"; xcf.add(this) }
    }
    // The removed Intel Apple desktop target is intentionally not declared.
    ...
    // The removed Intel tvOS target is intentionally not declared.
    ...
    // The removed Intel watchOS target is intentionally not declared.
    ...
    swiftExport {
        moduleName = "Ctor"
        flattenPackage = "io.github.kotlinmania.ctor"
    }

    android {
        namespace = "io.github.kotlinmania.ctor"
        compileSdk = 34
        minSdk = 24
...
mavenPublishing {
    ...
    coordinates(group.toString(), "ctor-kotlin", version.toString())

    pom {
        name.set("ctor-kotlin")
        description.set("Kotlin Multiplatform port of mmastrac/linktime - __attribute__((constructor)) for Rust")
        inceptionYear.set("2026")
        url.set("https://github.com/KotlinMania/ctor-kotlin")
        ...
        scm {
            url.set("https://github.com/KotlinMania/ctor-kotlin")
            connection.set("scm:git:git://github.com/KotlinMania/ctor-kotlin.git")
            developerConnection.set("scm:git:ssh://github.com/KotlinMania/ctor-kotlin.git")
        }
    }
}
...
val codeqlCompileJvm = tasks.register<JavaExec>("codeqlCompileJvm") {
    ...
    val sentinelDir = layout.buildDirectory.dir("generated/codeql-empty")
    val sentinelFile = sentinelDir.map { it.file("CodeqlEmptySourceSentinel.kt") }
    ...
    doFirst {
        ...
        val compileSources =
            if (sourceFiles.isNotEmpty()) {
                ...
                val generatedFile = sentinelFile.get().asFile
