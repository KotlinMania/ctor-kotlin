# Claude Code Project Instructions — ctor-kotlin

## Project Overview

This is **ctor-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`ctor`](https://crates.io/crates/ctor).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/ctor/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/ctor>
