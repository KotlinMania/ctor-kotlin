...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`ctor`](https://crates.io/crates/ctor).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `ctor` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

`#[ctor]` and `#[dtor]` proc-macros
...
```kotlin
dependencies {
    implementation("io.github.kotlinmania:ctor-kotlin:0.1.0-SNAPSHOT")
}
```
...
Original work copyrighted by the upstream `ctor` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
